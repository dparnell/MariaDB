...
- (void)testConnect
{
    ...
    XCTAssertNotNil(db, [error domain]);
    [db close: &error];
    XCTAssertNil(error, [error domain]);
}
...
- (void) testExecute
{
    ...
    XCTAssertEqual([db affectedRows], (UInt64)2, @"It should insert 2 rows");
...
- (void) testQuery {
    ...
    if(![db query: @"select * from query_test order by id" withCallback:^BOOL(NSDictionary *row, NSArray* fields) {
        ...
        XCTAssertEqualObjects(@"id", [fields objectAtIndex: 0], @"it should be 'id'");
        XCTAssertEqualObjects(@"my_name", [fields objectAtIndex: 1], @"it should be 'my_name'");
        return YES;
    } andError: &error]) {
        XCTFail([error domain]);
    }
    
    XCTAssertEqual((int)[rows count], 2, @"it should return 2 rows");
    NSDictionary* row1 = [rows objectAtIndex: 0];
    XCTAssertEqualObjects([NSNumber numberWithInt: 1], [row1 objectForKey: @"id"], @"the id of the first row should be the NSNumber 1");
    XCTAssertEqualObjects(@"First value", [row1 objectForKey: @"my_name"], @"the value of the 'my_name' field in the first row should be 'First value'");
    ...
    if(![db query: @"select current_timestamp" withCallback:^BOOL(NSDictionary *row, NSArray* fields) {
        ...
        XCTAssertEqualObjects(@"current_timestamp", [fields objectAtIndex: 0], @"it should be 'current_timestamp'");
        return YES;
    } andError: &error]) {
        XCTFail([error domain]);
    }
    id value = [[rows objectAtIndex: 0] objectForKey: @"current_timestamp"];
    XCTAssertTrue([value isKindOfClass: [NSDate class]], [NSString stringWithFormat: @"it should be a date - %@", value]);

    rows = [NSMutableArray new];
    if(![db query: @"select current_date" withCallback:^BOOL(NSDictionary *row, NSArray* fields) {
        [rows addObject: row];
        
        XCTAssertEqualObjects(@"current_date", [fields objectAtIndex: 0], @"it should be 'current_date'");
        return YES;
    } andError: &error]) {
        XCTFail([error domain]);
    }
    ...
    if(![db query: @"select current_time" withCallback:^BOOL(NSDictionary *row, NSArray* fields) {
        ...
        XCTAssertEqualObjects(@"current_time", [fields objectAtIndex: 0], @"it should be 'current_time'");
        return YES;
    } andError: &error]) {
        XCTFail([error domain]);
    }
    ...
    XCTAssertFalse([db execute: @"select * from foobar" withError: &error], @"the select should have failed");
    error = nil;
